...
import { distance, shiftToPositive } from "./utils";

// Expose for window debugging
declare global {
	interface Window {
		Graph: typeof Graph;
		ScaledGraphPoint: typeof ScaledGraphPoint;
	}
}
...
const maxDist: number = 100;
const divisions: number = 100;
const stepSize: number = 0.25;
const graphDivisionDist: number = maxDist / divisions;
const xThreshold: number = 0.1;
...
const result = plotFunction("y = x^2", "x", "#f00", true) as PlotResult;
const { plots } = result;
...
if (originalEl) {
	originalEl.textContent = `Original function: ${orig}`;
}
if (derivativeEl) {
	derivativeEl.textContent = `Taylor Expansion: ${taylor}`;
}
