import VectorArray from "../../../../packages/vector-array/VectorArray.js";
...
$(".infectionTableWrapper button.activateButton").on({
	async click() {
		...
		const upperBounds = new VectorArray.Entry(size, size),
			lowerBounds = new VectorArray.Entry(0, 0),
			$wrapper = $(this).parents(".infectionTableWrapper"),
			$this = $(this);
		...
		function addAvailableEdgeTiles(x, y) {
			...
			// Edge
			if (!checkOccupied(up) && checkIfOutOfBounds(up)) edgeTiles.add(up);
			if (!checkOccupied(down) && checkIfOutOfBounds(down)) edgeTiles.add(down);
			if (!checkOccupied(left) && checkIfOutOfBounds(left)) edgeTiles.add(left);
			if (!checkOccupied(right) && checkIfOutOfBounds(right)) edgeTiles.add(right);
			/*
			 * // Corner
