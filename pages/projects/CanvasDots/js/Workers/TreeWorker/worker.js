		...
		outer: while (nodeCount < MAX_NODES) {
			...
			for (let j = 0; j++ < SAMPLE_SIZE;) {
				const angle = getRandomInt(0, 360),
					index = getRandomInt(0, tree.nodes.length - 1),
					node = tree.nodes[index],
					dist = getRandomInt(MIN_DIST, MAX_DIST),
					x = Math.clamp(dist * Math.sin(angle) + node.x, 0 + CONSTANTS.MARGIN, canvasWidth - CONSTANTS.MARGIN),
					y = Math.clamp(dist * Math.cos(angle) + node.y, 0 + CONSTANTS.MARGIN, canvasHeight - CONSTANTS.MARGIN),
					candidate = new AbstractPoint({ x, y });
				let closest;
