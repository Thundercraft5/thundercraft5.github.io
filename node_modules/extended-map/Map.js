...
export default class Map extends globalThis.Map {
	...
	static Entry = class Entry {
		...
		constructor(key, value) {
			...
			else if (key instanceof this.constructor)
				return new this.constructor(key.key, key.value);
		...
		equals(otherEntry) {
			return this.#key === otherEntry.key && this.#value === otherEntry.value;
		}
		...
		* [Symbol.iterator]() {
		...
		toString() { return `Map.Entry(${ String(this.#key) } => ${ String(this.#value) })`; }
	...
	constructor(...entries) {
		...
		Object.getOwnPropertyNames(Map.prototype).forEach(k => typeof(this[k]) === 'function' 
	...
	set(k, v) {
		// Map.#assertThis(this);
		if (this.#locked) this.#assertNotLocked();

		if (k instanceof Array && k.length === 2 && this.#depth++ <= 0) 
			return this.set(...k);
	...
	has(key) {
		...
		if (key instanceof Map.Entry) return super.has(key.key) && this.every((_, v) => v === key.value);
	...
	hasEntry(entry) {
		return this.some((k, v) => entry.key === k && entry.value === v);
	}

	hasAllEntries(...entries) {
		return entries.every(entry => this.hasEntry(entry));
	}

	hasAnyEntry(...entries) {
		return entries.some(entry => this.hasEntry(entry));
	}
