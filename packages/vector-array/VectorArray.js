...
export default class VectorArray extends Array {
	static Entry = class Entry {
		 ...
		 */
		constructor(x, y) {
			if (Array.isArray(x) && x.length === 2 && this.#depth++ <= 0) return new this.constructor(x[0], x[1]);
			else if (x instanceof this.constructor) return new this.constructor(x.toArrayEntry());
	...
	#randomHelper(count, getter) {
		if (count > this.length || count < 1) throw new RangeError("Count must not be greater than size of map or less than 1");
	...
	constructor(...entries) {
		...
		if (entries.length === 1 && typeof entries[0] === "number") {
			...
			return this;
		} else if (entries.length === 1 && Array.isArray(entries[0]) && entries[0].every(v => Utils.isEntry(v))) [entries] = entries;

		entries.forEach((entry, i) => {
			const entryIsConstructed = entry instanceof VectorArray.Entry;

			if (entry.length !== 2 && !entryIsConstructed) throw new TypeError(`${ entry } is not a vector entry object`);
	...
	add(x, y) {
		...
		else if (this.has(x, y)) return this.add(this.remove(x, y));

		else {
			const i = this.findEmptyIndex();

			if (i !== null) this[i] = new VectorArray.Entry(x, y);
			else this[this.length++] = new VectorArray.Entry(x, y);
		}
	...
	mapWith(callbackfn, thisArg = this) {
		...
		entries.forEach(([x, y], i, arr) => {
			...
			if (!Utils.isEntry(res)) throw new TypeError(`The callback to ${
