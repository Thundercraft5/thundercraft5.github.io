...
export default class Utils {
	static assertFunction(v, nullable = false) {
		if (!(v instanceof Function) && (nullable ? v != null : true)) throw new TypeError(`${ Utils.toRepresentation(v) } is not a function`);
	...
	static toRepresentation(v) {
		...
		if (!v && v instanceof Boolean || v === null || v === undefined || isNaN(v)) ret = String(v);
		else if (v.__proto__.constructor.name !== "" && typeof v === "object") ret = `#<${ v[Symbol.toStringTag] || v.__proto__.constructor.name }>`;
		else if (typeof v === "object" && v[Symbol.toStringTag] || Array.isArray(v)) ret = Object.prototype.toString.call(v);
		else if (typeof v === "string") ret = `"${ v }"`;
		else ret = String(v);
